...
class DACS(EncoderDecoder):
    ...
    def init_log_vars(self,source=True,target=None):
        ...
        if source is not None:
            ...
            log_vars['decode.acc_src'] = torch.tensor(0.0)

        if target is not None:
            log_vars['decode.loss_tgt'] = torch.tensor(0.0)
            log_vars['decode.acc_tgt'] = torch.tensor(0.0)
    ...
    def train_and_update(self,optim_wrapper, log_vars,img,img_data_samples,seg_weight=None,suffix=""):
        seg_label = self.decode_head._stack_batch_gt(img_data_samples)
        ...
        loss = dict()
        loss[self.decode_head.loss_decode.loss_name] = self.decode_head.loss_decode(
                    seg_logits,
                    seg_label,
                    weight=seg_weight,
        ...
        loss['acc_seg'] = accuracy(
            seg_logits, seg_label, ignore_index=self.decode_head.ignore_index)
    ...
    def forward_train(self, img_dict, target_img_dict, optim_wrapper=None):
        ...
        for i in range(batch_size):
            mix_data_samples[i].gt_sem_seg.data = mixed_lbl[i]

        # train mixed image
        self.train_and_update(optim_wrapper,log_vars,mixed_img,mix_data_samples,mix_weight,suffix="tgt") '''

        mix_data_samples = deepcopy(tgt_data_samples)
        for i in range(batch_size):
            mix_data_samples[i].gt_sem_seg.data = pseudo_label[i].unsqueeze(0)
        self.train_and_update(optim_wrapper,log_vars,target_img,mix_data_samples,pseudo_weight,suffix="tgt") 
